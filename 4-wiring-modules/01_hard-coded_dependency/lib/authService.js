...
const db = require('./db'); // Get db handle
...
exports.login = (username, password, callback) => {
  users.get(username, (err, user) => {
    if(err) return callback(err); // Give control to the event loop, with the error
    ...
    bcrypt.compare(password, user.hash, (err, res) => {
      ...
      callback(null, token); // Give the control back to the event loop, with no error & the token.
    });
  });
};
